...
export default function CreateProjectModal({ show, onClose, categories = [], selectedYear, updateProjectData }) {
    ...
    useEffect(() => {
        if (show) {
            // Reset all form data to initial clean state
            reset();
            clearImages();
            setEngineers([{ name: '', titles: [''] }]);
            
            // Set default year
            const defaultYear = parseInt(selectedYear) || new Date().getFullYear();
            setData('project_year', defaultYear);
        }
    }, [show, reset, selectedYear, setData]);
    ...
    const submit = (e) => {
        ...
        post(route('projects.store'), {
            forceFormData: true,
            onSuccess: (page) => {
                // Keep DPWHLoading for exactly 2 seconds, then show toast
                setTimeout(() => {
                    // Hide loading after 2 seconds
                    setIsLoading(false);
                    
                    // Show success toast after loading disappears
                    ...
                    // Close modal and reset form after toast appears
                    setTimeout(() => {
                        reset();
                        clearImages();
                        setEngineers([{ name: '', titles: [''] }]);
            ...
            onError: (errors) => {
                // Keep DPWHLoading for exactly 2 seconds, then show error toast
                setTimeout(() => {
                    // Hide loading after 2 seconds
                    setIsLoading(false);
                    
                    // Show error toast after loading disappears
                    const errorMessages = Object.values(errors).flat();
                    if (errorMessages.length > 0) {
                        showErrorToast(errorMessages[0]);
                    } else {
                        showErrorToast('Failed to create project. Please check the form for errors.');
                    }
                }, 2000); // Exactly 2 seconds of DPWHLoading
            },
            onFinish: () => {
